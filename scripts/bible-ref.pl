...
With some changes by Matthew Thorley https://github.com/padwasabimasala
...
Usage: bible-ref [bible] [book] [chapter:[verse[-verse|,verse]]] {/pattern/in}*
...
Examples:
  ...
  bible-ref AV 1 jn /Jesus/	list verses in 1 John in the AV which contain Jesus
...
Additional Options:
  --wrap will wrap lines at 120 chars

Available bibles are: %%%
Pull requests and issues welcome at https://github.com/padwasabimasala/bible-ref
END

$SIG{PIPE} = 'IGNORE';

my $help = "Try `bible-ref --help' for more information.\n";

my $lastverse = 176; 	# The largest possible verse number (Psalm 119:176)
my $linelen = 120;	# Maximum line length
my $wrap = 0; # Print verses on a single line
...
my $bibles = $ENV{'ROCKS_BIBLIAE'} . '/WardThorley_bible-ref';
my $bible  = $ENV{'BIBLE'}  || "RSV";

my ($dos, $ds);

if (($^O eq "solaris") || ($^O eq "aix") || ($^O eq "linux") || ($^O eq "darwin")) {
